...
import type { ToolbarPartContext, ToolbarParts } from '../../parts'
...
const { classes, part, slotContext } = useClvComponent<ToolbarPartContext>('toolbar', props)
...
<template>
  <ToolbarRoot
    :class="classes(['clv-toolbar', `clv-toolbar--${orientation}`])"
      ...
      <slot
        ...
        :context="
          entry.type === 'separator'
            ? slotContext('separator', { entry, index, orientation })
            : slotContext('item', { entry, index, disabled: entry.disabled, orientation })
        "
      >
        <ToolbarSeparator
          v-if="entry.type === 'separator'"
          :class="classes('clv-toolbar__separator')"
          v-bind="part('separator', { entry, index, orientation })"
        />
        <ToolbarLink
          v-else-if="entry.type === 'link'"
          :class="classes('clv-toolbar__item')"
          :href="entry.href"
          :target="entry.target"
          :rel="entry.rel"
          :aria-disabled="entry.disabled || undefined"
          @click="entry.disabled && $event.preventDefault()"
          v-bind="part('item', { entry, index, disabled: entry.disabled, orientation })"
        >
          {{ entry.label }}
        </ToolbarLink>
        <ToolbarToggleGroup
        ...
        <ToolbarButton
          v-else
          :class="
            classes(['clv-toolbar__item', { 'clv-toolbar__item--destructive': entry.destructive }])
          "
          :disabled="entry.disabled"
        ...
        >
          <slot
            name="action"
            :entry="entry"
            :context="slotContext('item', { entry, index, disabled: entry.disabled, orientation })"
            >{{ entry.label }}</slot
          >
        </ToolbarButton>
      </slot>
